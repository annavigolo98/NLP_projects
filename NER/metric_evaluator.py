...
class MetricEvaluator(BaseModel):
    def compute_metrics(self, logits_and_labels):
            ...
            labels_str = [[label_names[t] for t in label if t  !=-100] for label in labels]
            preds_str = [[label_names[p] for p,t in zip(pred, label) if t !=-100 ] for pred, label in zip(preds, labels)]

            metric_evaluation = metric.compute(predictions = preds_str, references = labels_str)
